...
function add_event(){
    include('db_connect.php');
        ...
        if(!file_exists($_FILES['fileDoc']['tmp_name']) || !is_uploaded_file($_FILES['fileDoc']['tmp_name'])) {
            $insert="INSERT INTO `tbl_events`( `Event_title`, `Event_desc`, `date`, `Image`) 
...
function DeleteEvent(){
    ...
    $delet="DELETE FROM `tbl_events` WHERE Keycode=$key_code";
              //echo $insert;
...
function Editbird(){
    ...
    if($filedelete == null && !file_exists($_FILES['fileDoc']['tmp_name'])) {
        $insert="UPDATE `tbl_events` SET `Kingdom`='$Kingdom',`Phylum`='$Phylum',`Class`='$Class',
